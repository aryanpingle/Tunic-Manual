...
const CACHE_NAME = "tunic-cache-1";
/** @type {Cache} */
let CACHE = null;
...
self.addEventListener("fetch", (fetchEvent) => {
    fetchEvent.respondWith(
        ENABLE_CACHING ? get_request(fetchEvent) : fetch(fetchEvent.request)
    );
});

function isResourceRequest(urlString) {
    if (urlString.includes("/Tunic-Manual-Pages/")) return true;
 ...
 */
async function get_request(fetchEvent) {
    /** @type {Request} */
 ...
 * @param {Cache} cache
 */
async function networkFetchWithCache(request) {
 ...
 * @param {Request} request 
 */
async function requestForDocument(request) {
